...
class BeePlusSearch(
    ...
    def __init__(self, G: ProbDetGrammar[U, V, W]) -> None:
        ...
        self._cost_list: Dict[Tuple[Type, U], List[float]] = {}
        ...
        self._delayed: Dict[
            Tuple[Type, U],
            List[Tuple[Tuple[Type, U], List[int], DerivableProgram, Optional[int]]],
        ] = defaultdict(list)
    ...
    def _add_combination_(
        ...
        for i in to_check:
            ...
            if index_cost[i] >= len(self._cost_list[Si]):
                self._delayed[Si].append((S, index_cost, P, changed_index))
                return
    ...
    def _trigger_delayed_(self, Si: Tuple[Type, U]) -> None:

        elements = self._delayed[Si].copy()
        self._delayed[Si].clear()
        for S, index_cost, P, to_check in elements:
            self._add_combination_(S, P, index_cost, to_check)

    def _add_cost_(self, S: Tuple[Type, U], cost: float) -> Tuple[bool, int]:
        cost_list = self._cost_list[S]
    ...
    def _index_cost2real_cost_(
        ...
        for i in range(self.G.arguments_length_for(S, P)):
            out += self._cost_list[self._non_terminal_for_(S, P, i)][indices[i]]
        return out
...
def enumerate_prob_grammar(G: ProbDetGrammar[U, V, W]) -> BeePlusSearch[U, V, W]:
